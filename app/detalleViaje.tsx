...
export default function DetalleViaje() {
  ...
  const [cargando,
    setCargando] = useState(true);
  const [data,
    setData] = useState < any > (null);
  const [procesandoPdf,
    setProcesandoPdf] = useState(false);
  const [coordenadasRuta,
    setCoordenadasRuta] = useState < number[][] > ([]);
    ...
    try {
      ...
      if (jornada && jornada.ruta_geojson) {
        try {
          ...
          ? JSON.parse(jornada.ruta_geojson): jornada.ruta_geojson;
      ...
      if (!esPro) {
        Alert.alert("Función Premium ⭐", "La generación de reportes PDF oficiales es exclusiva para usuarios PRO.",
          [{
            ...
            text: "VER PLANES", onPress: () => router.push('/PantallaSuscripcion')
          }]
        );
        return;
      }
    ...
    try {
      ...
      try {
        ...
        if (inspeccionesDB.length > 0) {
          ...
          let items = {};
          try {
            items = JSON.parse(ultimaInspeccion.detalles_json || '{}');
          } catch (e) {}
        ...
        console.log("Error leyendo inspección", e);
      }
  ...
  if (!data || !data.jornada) {
    ...
    }} > No se encontró el viaje. < /Text> < /View>
    );
  }
  ...
  return ( < View style = {
  ...
  } >
    {
      ...
      styles.header
        ...
        padding: 5
      }} > < MaterialCommunityIcons name = "close-circle" size = {
      28
    } color = {
      COLORS.subtext
      ...
      styles.sectionTitle
    } > Ruta Recorrida < /Text> < View style = {
      styles.mapContainer
    } >
    {
      coordenadasRuta.length > 1 ? ( < Mapbox.MapView style = {{
        flex: 1
      }} styleURL = {
        ...
        /> < Mapbox.ShapeSource id = {
          `routeSource-${jornada.id}`
        } shape = {{
            type: 'Feature', geometry: {
              type: 'LineString', coordinates: coordenadasRuta
            }, properties: {}
          }} > < Mapbox.LineLayer id = {
          `routeLayer-${jornada.id}`
        } style = {{
            lineColor: '#3b82f6', lineWidth: 5, lineCap: 'round', lineJoin: 'round'
          }} /> < /Mapbox.ShapeSource> < Mapbox.ShapeSource id = {
          `start-${jornada.id}`
        } shape = {{
            type: 'Feature', geometry: {
              type: 'Point', coordinates: coordenadasRuta[0]
            }, properties: {}
          }} > < Mapbox.CircleLayer id = {
          `startLayer-${jornada.id}`
        } style = {{
            circleRadius: 7, circleColor: '#22c55e', circleStrokeWidth: 2, circleStrokeColor: '#ffffff'
          }} /> < /Mapbox.ShapeSource> < Mapbox.ShapeSource id = {
          `end-${jornada.id}`
        } shape = {{
            type: 'Feature', geometry: {
              type: 'Point', coordinates: coordenadasRuta[coordenadasRuta.length - 1]
            }, properties: {}
          }} > < Mapbox.CircleLayer id = {
          `endLayer-${jornada.id}`
        } style = {{
            circleRadius: 7, circleColor: '#ef4444', circleStrokeWidth: 2, circleStrokeColor: '#ffffff'
          }} /> < /Mapbox.ShapeSource> < /Mapbox.MapView>
      ): ( < View style = {{
          flex: 1, justifyContent: 'center', alignItems: 'center', backgroundColor: '#111'
        }} > < Text style = {{
          color: COLORS.subtext
        }} > Ruta vacía o viaje demasiado corto < /Text> < /View>
      )} < /View> < Text style = {
      styles.sectionTitle
    } > Información < /Text> < View style = {
      styles.infoCard
    } > < InfoRow label = "Unidad:" value = {
      jornada.unidad
    } /> < InfoRow label = "Operador:" value = {
      jornada.operador
    } /> < InfoRow label = "Inicio:" value = {
      new Date(jornada.fecha_inicio).toLocaleString()} /> < InfoRow label = "Fin:" value = {
      jornada.fecha_fin ? new Date(jornada.fecha_fin).toLocaleString(): 'En curso'
    } /> < InfoRow label = "Origen:" value = {
      jornada.origen
    } /> < InfoRow label = "Destino:" value = {
      jornada.destino
    } /> < /View>
    ...
    {
      procesandoPdf ? < ActivityIndicator color = "#000" />: < MaterialCommunityIcons name = "file-pdf-box" size = {
        20
      } color = "#000" style = {{
          marginRight: 8
        }} />
    ...
    } < /Text> < /TouchableOpacity> < TouchableOpacity style = {
      styles.btnDanger
    } onPress = {
      handleEliminar
    } > < MaterialCommunityIcons name = "trash-can-outline" size = {
      20
    } color = {
      COLORS.danger
    } style = {{
    ...
    } > ELIMINAR REGISTRO < /Text> < /TouchableOpacity> < View style = {{
        height: 40
      }} /> < /ScrollView> < /View>
  );
}
